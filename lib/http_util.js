 ...
 * @param {failure} (Optional) This callback handles failure case. It is called when the
 *     request failed or the response code is not 200. This has 2 parameters.
 *     One is a status code of response (if undefined, -1 returned). And the other is
 *     a requested url.
 **/
exports.get = function(url, encoding, success, failure) {
    if (url && encoding && success) {
        var reg = /^\.\/|^\.\.\//;
        if (url && 0 === url.search(reg)) {
            fs.readFile(url, function(err, body) {
                if (err) {
                    if (failure)  { failure(-1, err, url); }
                } else {
            ...
            request({ uri: url, encoding: null },
                    function(error, response, body) {
                        if (!error && response.statusCode === 200) {
                            ...
                            if (failure) { failure(response ? response.statusCode : -1, error, url); }
                        }
                    });
        }
    }
 ...
 *    return [null, "abcdef"] }).join(function(_) {
 *    return item.join(function(x) {
 *        return item.join(function(_) {
 *            return item.join(function(y) {
 *                return [[x[0], y[0]], y[1]];
 *            }, _[1]);
 *        }, x[1]);
 *    }, _[1]);
...
exports.monad = function(f) {
    f.isMonad = true;
    f.join = function(g, val) { return g(this(val)); };
 ...
 *     parser]). The output function takes their variables as a argument.
        ...
        return (function() {
            ...
            var rec = function(i, val) {
                if (i === args.length - 1) {
                    ...
                    if (args[i][1].isMonad) {
                        return args[i][1].join(function(_) {
                            var name = args[i][0];
                            if (name) { ret[name] = _[0]; }
                            return rec(i + 1, _[1]);
                        }, val);
                    } else {
                        // silently ignore unless args[i][1] is monad
                        return rec(i + 1, val);
                    }
                }
            };
